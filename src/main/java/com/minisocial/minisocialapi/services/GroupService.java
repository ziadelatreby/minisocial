...
public class GroupService {
    ...
    //@abdelrahman recommendation to make it transactional
    @Transactional
    ...
    public void rejectJoinRequest(Long groupId, Long targetUserId, Long ctxUserId) {
        ...
        public void kickMember(Long groupId, Long targetUserId, Long ctxUserId) {
            ...
            // Notify the user about the acceptance
            Notification userNotification= NotificationFactory.createGroupLeaveNotification(  targetUser,targetUser, group, true);
    ...
    public List<UserDTO> getGroupJoinRequests(Long groupId, Long ctxUserId) {

        if (!groupRepository.isUserAdminInGroup(ctxUserId, groupId)) {
            throw new NotAuthorizedException("Only admins can view join requests");
        }

        List<User> joinRequests = groupRepository.getJoinRequestsForGroup(groupId);

        return joinRequests.stream().map(User::toDTO).collect(Collectors.toList());
    }
